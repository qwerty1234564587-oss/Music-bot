...
# Premium animated emoji (Telegram Premium users see the animation,
# others fall back to the plain emoji placed right after the id).
EMOJI_SPARKLE = '<emoji id=5201873447554145566>✨</emoji>'
EMOJI_CHECK = '<emoji id=5409098988156629257>✅</emoji>'
EMOJI_ALERT = '<emoji id=5422814644093868925>🔴</emoji>'

CAPTION_TXT = """
{sparkle} <b>ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ {chat_title}</b> {sparkle}
▰▰▰▰▰▰▰▰▰▰▰▰▰
{check} <b>Nᴀᴍᴇ</b> ➻ {mention}
{check} <b>Iᴅ</b> ➻ <code>{uid}</code>
{check} <b>Usᴇʀɴᴀᴍᴇ</b> ➻ @{uname}
{check} <b>Mᴇᴍʙᴇʀ #</b> ➻ {count}
▰▰▰▰▰▰▰▰▰▰▰▰▰
{sparkle} <i>Eɴᴊᴏʏ ʏᴏᴜʀ sᴛᴀʏ, ᴜsᴇ /help ғᴏʀ ᴄᴏᴍᴍᴀɴᴅs!</i> {sparkle}
"""
...
async def toggle(client, m: Message):
    ...
    status_emoji = EMOJI_CHECK if flag == "on" else EMOJI_ALERT
    await m.reply_text(
        f"{status_emoji} <b>{'Eɴᴀʙʟᴇᴅ' if flag == 'on' else 'Dɪsᴀʙʟᴇᴅ'} Wᴇʟᴄᴏᴍᴇ Iɴ {m.chat.title}</b>",
...
async def welcome(client, update: ChatMemberUpdated):
    ...
    if burst >= JOIN_THRESHOLD:
        ...
        await safe_send(
            client.send_message,
            cid,
            f"{EMOJI_ALERT} <b>Mᴀssɪᴠᴇ Jᴏɪɴ Dᴇᴛᴇᴄᴛᴇᴅ ( x{burst} ). Wᴇʟᴄᴏᴍᴇ ᴍᴇssᴀɢᴇs ᴅɪsᴀʙʟᴇᴅ ғᴏʀ {minutes} ᴍɪɴᴜᴛᴇs.</b>",
            parse_mode=ParseMode.HTML,
        )
        return
